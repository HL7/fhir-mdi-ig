...
**Specimens received but not analyzed:** Specimens provided to a laboratory that are **not** tested, and therefore do not provide any resultant information on the DiagnosticReport, should be **excluded** from DiagnosticReport. However, ME/Cs may need to know if a forensic toxicology laboratory received a specimen but did not analyze it and the reason. In such cases an Observation - Toxicology Lab Result must reference the Specimen - Toxicology Lab and must provide a reason for no analysis (dataAbsentReason). Additionally, the Specimen - Toxicology Lab may use the Specimen.condition to describe the state of the specimen via codes from the extensible value set [hl7VS-specimenCondition](https://terminology.hl7.org/ValueSet-v2-0493.html) and/or use the Specimen.note to describe details or issues about the specimen.
